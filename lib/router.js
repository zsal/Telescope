...
DataController = RouteController.extend({
  onBeforeAction: function(pause) {
    ...
    _.each(this.routeData(), function (dataItem) {
      if (dataItem.cursor && dataItem.wait && !dataItem.cursor.count()) {
      /*  
        If:
        1. This data item has a specific cursor.
        2. We should wait on it.
        3. The cursor returns an empty count.
        Then:
      */  
        if (dataItem.subscription.ready()) {
            // The subscription is ready yet the cursor is still empty.
            notFound = true;
        } else {
    ...
    // Case 1: at least one data item was not found
    if (notFound) {
      this.render(this.lookupProperty('notFoundTemplate')); // render notFound template
      return pause(); // pause route
    }

    // Case 2: at least one data item is still loading
    if (loading) {
      this.render(this.lookupProperty('loadingTemplate')); // render loading template
      return pause(); // pause route
    }

    // Case 3: no data items are loading or not found; proceed to route
    
  },
  data: function() { // extract just the cursors from the routeData object
    var data = {};
    _.each(this.routeData(), function (dataItem) {
...
PostPageController = DataController.extend({
  routeData: function() {
    return [
      {
        name: 'post', // the name of the cursor returned
        subscription: Meteor.subscribe('singlePost', this.params._id), // the subscription providing the data
        cursor: Posts.find({_id: this.params._id}), // the cursor using the data
        persist: true, //  Subscription Manager style subscription persistence? not used yet
        wait: true // display loading template until all data is loaded
      },
      {
        name: 'postComments',
        ...
        wait: false // render route first; then load data
      },
      {
        name: 'postUsers',
        subscription: Meteor.subscribe('postUsers', this.params._id),
        cursor: null, // we don't want to return a cursor, just preload data
        wait: false
      }
    ]
  }
